...
export function calculateLineTotals(line: ProductLine) {
  ...
  // 1. Unit VAT based on Unit TP
  const unitVat = Number((unitTp * vatRate / 100).toFixed(2));
  
  // 2. Total TP based on Unit TP * Qty
  const totalTp = Number((unitTp * quantity).toFixed(2));
  
  // 3. Total VAT based on Total TP * VAT Rate (as per image logic)
  const totalVat = Number((totalTp * vatRate / 100).toFixed(2));
  
  // 4. Total Amount (TP+Vat) column
  // As per user request: Total TP + Total VAT
  const totalPrice = Number((totalTp + totalVat).toFixed(2));
